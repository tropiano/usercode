...
std::pair<RooFitResult*, RooRealVar*> TagAndProbeAnalyzerElectron::fit(RooAbsData* data, const char* name, std::string option, unsigned int bin) {
	...
	RooVoigtian voigtian_pass("voigtian_pass", "voigtian_pass", _mass, el_pass, width_pass, sigma_pass);
	RooCBShape crystal_pass("crystalball_pass", "crystalball_pass", _mass, el_pass2, sigma_pass2, a_pass, n_pass);
	RooAddPdf signal_pass("signal_pass", "signal_pass", voigtian_pass, crystal_pass, fraction_pass);

	RooVoigtian voigtian_fail("voigtian_fail", "voigtian_fail", _mass, el_fail, width_fail, sigma_fail);
	RooCBShape crystal_fail("crystalball_fail", "crystalball_fail", _mass, el_fail2, sigma_fail2, a_fail, n_fail);
