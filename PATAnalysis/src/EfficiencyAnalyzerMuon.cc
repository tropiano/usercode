...
EfficiencyAnalyzerMuon::EfficiencyAnalyzerMuon(TFile* input, TFile* output, string dirname, string dirname1, string dataset, TFile* training_signal, TFile* training_background): 
...
void EfficiencyAnalyzerMuon::analyze(int bin, std::string fitOptions){
  
  if(_dirname!=""){
  if (!_dir) {
    std::cout << "### Error in EfficiencyAnalyzerMuon, dir \""<<_dirname.c_str()<<"\" not found in file"<< std::endl;
    return;
  }
  }
  ...
  TagAndProbeAnalyzerMuon tpana(_dir, _output, _dir1, _dataset, true, _training_signal, _training_background);
