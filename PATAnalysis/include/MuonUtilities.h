...
// Single Muon Candidate: it is true if the Muon is in the Acceptance (geometrical and kinematic) region (SYM)
...
// Single Muon Candidate: it is true if the Muon is in the Acceptance (geometrical and kinematic) region (ASYM, leg 0)
...
// Single Muon Candidate: it is true if the Muon is in the Acceptance (geometrical and kinematic) region (ASYM, leg 1)
...
// Muon Trigger Available: it is true if the trigger (existing in a run range) was run in the specified run or in the complete run set
...
// is Muon Triggered: it is true if the trigger (existing in a run range) was Accepted in the specified run or in the complete run set
...
*/
	const TriggerObjectStandAloneCollection MatchMuon = Muon.triggerObjectMatchesByFilter("");
...
*/
	...
	std::cout<<"matched muons size: "<<MatchMuon.size()<<std::endl;
...
inline bool RecSelected(string Flag, const std::vector<reco::CompositeCandidate>& ZREC, const pat::TriggerEvent& triggers, int run, double rho){
	...
	if(!zdaughters.size()){
		...
		if(Flag=="_AccSYM"){
			...
			if(dau0->muonID(muID_SYM.c_str())==1.0){
				muon_ID0 = true;
			}
			if(dau1->muonID(muID_SYM.c_str())==1.0){
				muon_ID1 = true;
			}
			...
			if(dau0->muonID(muID_ASYM0.c_str())==7.0 || 
			   dau0->muonID(muID_ASYM0.c_str())==1.0 || 
			   dau0->muonID(muID_ASYM0.c_str())==3.0 || 
			   dau0->muonID(muID_ASYM0.c_str())==5.0){ 
				muon_ID0 = true;
			}
			if(dau1->muonID(muID_ASYM1.c_str())==7.0 || 
			   dau1->muonID(muID_ASYM1.c_str())==1.0 || 
			   dau1->muonID(muID_ASYM1.c_str())==3.0 || 
			   dau1->muonID(muID_ASYM1.c_str())==5.0){
				muon_ID1 = true;
			}
			return muon_ID0 && muon_ID1;
		}
...
inline bool RecSelected_mod(string Flag, const std::vector<reco::CompositeCandidate>& ZREC, const pat::TriggerEvent& triggers, int run, double rho){
	...
	if(!zdaughters.size()){
		...
		if(Flag=="_AccSYM"){
			...
			if(muTrgMatchReq==true){
				if(isMuonTriggered(triggers, run) && 
				   (RecSelected_TrgMatch(*dau0, triggers, run))){ 
					cutTrg=true;
				}
			}
			else if(muTrgMatchReq==false){
...
template<class MUON> bool IsoJet(const std::vector<const MUON*>& muons, const reco::Jet& jet){
	...
	if(MinDeltaRZDau<MUON>(muons,jet) < isojetcut){
		iso_jet = false;
	}
	return iso_jet;
}
...
inline bool singleMu_Tag_SYM(const reco::Candidate& cand, int run, double rho){
	...
	if(muon){
		...
		if(muonIsoRho <= SYM_TAG_muonIsoRhoCut){
			TAG_iso = true;
		}
 
		if(SYM_TAG_MuID!=""){
			TAG_MuID = false;
			if(muon->muonID(SYM_TAG_MuID.c_str())==1.0){
				TAG_MuID = true;
			}
		}
...
// Tag cuts ASYM
// Cuts applied on Tag Muon: ASYM, leg 0
inline bool singleMu_Tag_ASYM0(const reco::Candidate& cand, int run, double rho){
	...
	if(muon){
		...
		if(muon::isGoodMuon(*muon, muon::GlobalMuonPromptTight) && 
		   ...
		   ptRelErr <= ASYM0_TAG_maxPtRelErr){
			TAG_qual = true;
		}
		...
		float muonIsoRho = (iso03.sumPt + max(0.,iso03.emEt-Aecal*(rho)) + max(0.,iso03.hadEt-Ahcal*(rho)))/muon->pt();
		if(muonIsoRho <= ASYM0_TAG_muonIsoRhoCut){
			TAG_iso = true;
		}
 
		if(ASYM0_TAG_MuID!=""){
			TAG_MuID = false;
			if(muon->muonID(ASYM0_TAG_MuID.c_str())==1.0){
				TAG_MuID = true;
			}
		}
...
// Cuts applied on Tag Muon: ASYM, leg 1
inline bool singleMu_Tag_ASYM1(const reco::Candidate& cand, int run, double rho){
	...
	if(muon::isGoodMuon(*muon, muon::GlobalMuonPromptTight) &&
	   ...
	   ptRelErr <= ASYM1_TAG_maxPtRelErr){
		TAG_qual = true;
	}
	...
	float muonIsoRho = (iso03.sumPt + max(0.,iso03.emEt-Aecal*(rho)) + max(0.,iso03.hadEt-Ahcal*(rho)))/muon->pt();
	if(muonIsoRho <= ASYM1_TAG_muonIsoRhoCut){
		TAG_iso = true;
	}
 
	if(ASYM1_TAG_MuID!=""){
		TAG_MuID = false;
		if(muon->muonID(ASYM1_TAG_MuID.c_str())==1.0){
			TAG_MuID = true;
		}
	}
...
// Probe cuts SYM
// Single Muon cut - Acceptance (geometrical and kinematic): SYM
...
inline bool singleMu_Probe_Trg_SYM(const reco::Candidate& cand, int run, double rho){
	...
	if(muTrgMatchReq){
		TPTrgMatch=false;
		if(muon){
			//if(RecSelected_TrgMatch(*muon, run))
			TPTrgMatch=true;
		}
		else{
			TPTrgMatch=false;
...
// Single Muon cut - Impact Parameter: SYM
...
// Single Muon cut - Isolation with rho: SYM
...
// Single Muon cut - Isolation without rho: SYM
...
inline bool singleMu_Probe_MuID_SYM(const reco::Candidate& cand, int run, double rho){
	...
	if(muon){
		bool muon_ID = false; 
		if(muon->muonID(muID_SYM.c_str())==1.0){
			muon_ID = true;
		}
		return muon_ID;
	}
...
// Single Muon cut - Trigger: ASYM, leg0
...
// Per il momento non applico il probe Trg al secondo muone
// Single Muon cut - Trigger: ASYM, leg1
...
// Single Muon cut - Quality: ASYM, leg0
...
// Single Muon cut - Quality: ASYM, leg1
...
// Single Muon cut - Isolation with rho: ASYM, leg0
inline bool singleMu_Probe_Iso_ASYM0(const reco::Candidate& cand, int run, double rho){
	...
	if(mu0){
		...
		const reco::MuonIsolation& iso03_0 = mu0->isolationR03(); 
		float muEta0 = mu0->eta(); 
		float Aecal0 = cAecalEE_ASYM0;
		float Ahcal0 = cAhcalHE_ASYM0;
		if (fabs(muEta0) < maxmuEta_ASYM0) {
			Aecal0 = cAecalEB_ASYM0;
		...
		float muonIsoRho0 = (iso03_0.sumPt + max(0.,iso03_0.emEt-Aecal0*(rho)) + max(0.,iso03_0.hadEt-Ahcal0*(rho)))/mu0->pt();

		if(muonIsoRho0 <= muonIsoRhoCut_ASYM0){
			iso0 = true;
		}
		return iso0;
	}
...
// Single Muon cut - Isolation with rho: ASYM, leg1
inline bool singleMu_Probe_Iso_ASYM1(const reco::Candidate& cand, int run, double rho){
	...
	if(mu1){
		...
		const reco::MuonIsolation& iso03_1 = mu1->isolationR03(); 
		float muEta1 = mu1->eta(); 
		float Aecal1 = cAecalEE_ASYM1;
		float Ahcal1 = cAhcalHE_ASYM1;
		...
		float muonIsoRho1 = (iso03_1.sumPt + max(0.,iso03_1.emEt-Aecal1*(rho)) + max(0.,iso03_1.hadEt-Ahcal1*(rho)))/mu1->pt();

		if(muonIsoRho1 <= muonIsoRhoCut_ASYM1){
			iso1 = true;
		}
		return iso1;
	}
...
// Single Muon cut - Isolation without rho: ASYM, leg0
inline bool singleMu_Probe_Iso_ASYM0_mod(const reco::Candidate& cand, int run, double rho){
	...
	if(mu0){
		...
		const reco::MuonIsolation& iso03 = mu0->isolationR03(); 
		float muEta = mu0->eta(); 
		float Aecal = cAecalEE_ASYM0;
		float Ahcal = cAhcalHE_ASYM0;
		if (fabs(muEta) < maxmuEta_ASYM0){
		...
		float muonIso = (iso03.sumPt + iso03.emEt + iso03.hadEt)/mu0->pt();

		if(muonIso <= muonIsoCut_ASYM0){
			iso0 = true;
		}
	 	return iso0;
	}
...
// Single Muon cut - Isolation without rho: ASYM, leg1
inline bool singleMu_Probe_Iso_ASYM1_mod(const reco::Candidate& cand, int run, double rho){
	...
	if(mu1){
		...
		const reco::MuonIsolation& iso03 = mu1->isolationR03(); 
		float muEta = mu1->eta(); 
		float Aecal = cAecalEE_ASYM1;
		float Ahcal = cAhcalHE_ASYM1;
		if (fabs(muEta) < maxmuEta_ASYM1) {
		...
		float muonIso = (iso03.sumPt + iso03.emEt + iso03.hadEt)/mu1->pt();

		if(muonIso <= muonIsoCut_ASYM1){
			iso1 = true;
		}
	 	return iso1;
	}
...
// Single Muon cut - Muon ID: ASYM, leg0
inline bool singleMu_Probe_MuID_ASYM0(const reco::Candidate& cand, int run, double rho){
	...
	if(mu0){
		...
		if(mu0->muonID(muID_ASYM0.c_str()) == 7.0 || 
		   mu0->muonID(muID_ASYM0.c_str()) == 1.0 || 
		   mu0->muonID(muID_ASYM0.c_str()) == 3.0 || 
		   mu0->muonID(muID_ASYM0.c_str()) == 5.0){
			mu0_ID = true;
		}
		return mu0_ID;
	}
...
// Single Muon cut - Muon ID: ASYM, leg1
inline bool singleMu_Probe_MuID_ASYM1(const reco::Candidate& cand, int run, double rho){
	...
	if(mu1){
		...
		if(mu1->muonID(muID_ASYM1.c_str()) == 7.0 || 
		   mu1->muonID(muID_ASYM1.c_str()) == 1.0 || 
		   mu1->muonID(muID_ASYM1.c_str()) == 3.0 || 
		   mu1->muonID(muID_ASYM1.c_str()) == 5.0){
			mu1_ID = true;
		}
		return mu1_ID;
	}
...
// Single Muon cut - All Selection: SYM
...
// Single Muon cut - All Selection: ASYM, leg0
...
// Single Muon cut - All Selection: ASYM, leg1
