...
//	string Address_IsoRho = "IsoRho_NO/"; 
	...
	double nmaxX = 6.0;
	double nminY = 1.0; 
	double nmaxY = 10000000.0;
	
	//Legenda
	string Leg_NUM = "#splitline{2 #mu overlap = true}{with a rec Z}";
	string Leg_DEN = "#splitline{no overlap requirement}{with a rec Z}";
...
Dir->cd();
	...
	TCanvas *RecJets = new TCanvas();
	RecJets->SetLogy();	
        RecJets->Range(0,0,25,25);
        RecJets->SetLeftMargin(0.160);
	...
	histo_NUM->GetXaxis()->SetTitle("Number of rec Jets");
	...
	RecJets->Write(rate.c_str());
	RecJets->SaveAs((Address_totale_out + pre_file_out + und_rate_und + cartella + und_DeltaR_0ex + pdf).c_str());
	RecJets->Close();
...
gStyle->SetOptStat(0);
	...
	histoDiv_NUM->GetYaxis()->SetRangeUser(nminY, nmaxY);
	histoDiv_NUM->GetXaxis()->SetTitle("Number of rec Jets");
