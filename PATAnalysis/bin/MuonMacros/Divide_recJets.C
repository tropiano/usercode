...
Jet->cd();
        ...
        //histo NUM
	TH1D* histo_NUM = (TH1D*) file->Get("RecoMuon/DoubleMu_Overlap/recoJetCounterTrue");
        //histo DEN
	TH1D* histo_DEN = (TH1D*) file->Get("RecoMuon/DoubleMu_Overlap/recoJetTotalCounter");
...
//        RecJets->SaveAs((outputpdf.c_str()));
//        RecJets->SaveAs((outputpng.c_str()));
//        RecJets->SaveAs((outputroot.c_str()));
//        RecJets->SaveAs((outputC.c_str()));

	string recjets = "recJets_";
        string rate = "rate_";
        string png = ".png";
	RecJets->Write(recjets.c_str());
        RecJets->SaveAs((recjets + rate + cartella + DeltaR + png).c_str());
	RecJets->Close();
	...
	string divideplot = ": Contamination ratio";
...
gStyle->SetOptStat(0);
	...
	Divide->Write(divide.c_str());
        Divide->SaveAs((recjets + divide + cartella +  DeltaR + png).c_str());
	Divide->Close();
