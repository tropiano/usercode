...
void Signal(string sample, string selections){
        ...
        TFile *signal_file = TFile::Open("/data/sfrosali/Zjets/CMSSW_4_2_5/src/Firenze/PATAnalysis/bin/DYtoLL_Train.root");
        
        //Output
        string out = "MCEff_DYtoLL_Test";
        string output = out;
        output+=".root";
...
TDirectory *GEff, *REff, *Acc, *Pt_Eff, *Eta_Eff, *RecoInclJetWGA_Eff, *RecoExclJetWGA_Eff, *RecoInclJet_Eff, *RecoExclJet_Eff, *R_RecoInclJetWGA_Eff, *R_RecoExclJetWGA_Eff, *R_RecoInclJet_Eff, *R_RecoExclJet_Eff, *Mass_Eff, *Dist, *GenRec, *IsoJet, *ChargeMisID;
...
ChargeMisID = outplots->mkdir("ChargeMisID");
   ...
   TH1D* AccDenom_genMassZ = (TH1D*) signal_file->Get("EfficiencyMuon/AccDenom_genMassZ");
   TH1D* AccDenom_genPtZ = (TH1D*) signal_file->Get("EfficiencyMuon/AccDenom_genPtZ");
   TH1D* AccDenom_genEtaZ = (TH1D*) signal_file->Get("EfficiencyMuon/AccDenom_genEtaZ");
   TH1D* AccDenom_RecoInclJetNumber = (TH1D*) signal_file->Get("EfficiencyMuon/AccDenom_RecoInclJetNumber");
   TH1D* AccDenom_RecoExclJetNumber = (TH1D*) signal_file->Get("EfficiencyMuon/AccDenom_RecoExclJetNumber");
	
   TH1D* EffDenom_genMassZ = (TH1D*) signal_file->Get("EfficiencyMuon/EffDenom_genMassZ");
   TH1D* EffDenom_genPtZ = (TH1D*) signal_file->Get("EfficiencyMuon/EffDenom_genPtZ");
   TH1D* EffDenom_genEtaZ = (TH1D*) signal_file->Get("EfficiencyMuon/EffDenom_genEtaZ");
   TH1D* EffDenom_RecoInclJetNumber = (TH1D*) signal_file->Get("EfficiencyMuon/EffDenom_RecoInclJetNumber");
   TH1D* EffDenom_RecoExclJetNumber = (TH1D*) signal_file->Get("EfficiencyMuon/EffDenom_RecoExclJetNumber");
	
        //Reco in acc. efficiency
    
	string genMassZEff_name = "EfficiencyMuon/genMassZEff";
	...
	string genPtZEff_name = "EfficiencyMuon/genPtZEff";
	...
	string genEtaZEff_name = "EfficiencyMuon/genEtaZEff";
	...
	string RecoInclJetEffWGA_name = "EfficiencyMuon/RecoInclJetEffWGenAcc";
	...
	string RecoExclJetEffWGA_name = "EfficiencyMuon/RecoExclJetEffWGenAcc";
	...
	string RecoInclJetEff_name = "EfficiencyMuon/RecoInclJetEff";
	...
	string RecoExclJetEff_name = "EfficiencyMuon/RecoExclJetEff";
   ...
   //Global Efficiency calculation vs Z Mass
	...
	effincl<<"Global efficiency (Acc+Trg+Imp+Qual+Iso+MuID) vs inclusive multiplicity (Gen(Acc) NOT REQUIRED)"<<endl<<endl;
        for(int i=0; i<7; i++){
        double x = 0., y = 0.;
        ...
        effexcl<<"Global efficiency (Acc+Trg+Imp+Qual+Iso+MuID) vs exclusive multiplicity (Gen(Acc) NOT REQUIRED)"<<endl<<endl;
        for(int i=0; i<7; i++){
        double x = 0., y = 0.;
        ...
        effincl<<"Global efficiency (Acc+Trg+Imp+Qual+Iso+MuID) vs inclusive multiplicity (Gen(Acc) REQUIRED)"<<endl<<endl;
        for(int i=0; i<7; i++){
        double x = 0., y = 0.;
        ...
        effexcl<<"Global efficiency (Acc+Trg+Imp+Qual+Iso+MuID) vs exclusive multiplicity (Gen(Acc) REQUIRED)"<<endl<<endl;
        for(int i=0; i<7; i++){
        double x = 0., y = 0.;
	...
	LegEffZM->AddEntry(&Eff_ZMass_1234,"Acc+Trg+Imp+Qual","lp");
	LegEffZM->AddEntry(&Eff_ZMass_12345,"Acc+Trg+Imp+Qual+Iso","lp");
	LegEffZM->AddEntry(&Eff_ZMass_123456,"Acc+Trg+Imp+Qual+Iso+MuID","lp");
	LegEffZM->Draw();
	...
	LegEffZPt->AddEntry(&Eff_ZPt_1234,"Acc+Trg+Imp+Qual","lp");
	LegEffZPt->AddEntry(&Eff_ZPt_12345,"Acc+Trg+Imp+Qual+Iso","lp");
	LegEffZPt->AddEntry(&Eff_ZPt_123456,"Acc+Trg+Imp+Qual+Iso+MuID","lp");
	LegEffZPt->Draw();
	...
	LegEffZEta->AddEntry(&Eff_ZEta_1234,"Acc+Trg+Imp+Qual","lp");
	LegEffZEta->AddEntry(&Eff_ZEta_12345,"Acc+Trg+Imp+Qual+Iso","lp");
	LegEffZEta->AddEntry(&Eff_ZEta_123456,"Acc+Trg+Imp+Qual+Iso+MuID","lp");
	LegEffZEta->Draw();
	...
	LegEffInclJetWGA->AddEntry(&Eff_RecoInclJetWGA_1234,"Acc+Trg+Imp+Qual","lp");
	LegEffInclJetWGA->AddEntry(&Eff_RecoInclJetWGA_12345,"Acc+Trg+Imp+Qual+Iso","lp");
	LegEffInclJetWGA->AddEntry(&Eff_RecoInclJetWGA_123456,"Acc+Trg+Imp+Qual+Iso+MuID","lp");
	...
	LegEffExclJetWGA->AddEntry(&Eff_RecoExclJetWGA_1234,"Acc+Trg+Imp+Qual","lp");
	LegEffExclJetWGA->AddEntry(&Eff_RecoExclJetWGA_12345,"Acc+Trg+Imp+Qual+Iso","lp");
	LegEffExclJetWGA->AddEntry(&Eff_RecoExclJetWGA_123456,"Acc+Trg+Imp+Qual+Iso+MuID","lp");
	...
	LegEffRecoInclJ->AddEntry(&Eff_RecoInclJet_1234,"Acc+Trg+Imp+Qual","lp");
	LegEffRecoInclJ->AddEntry(&Eff_RecoInclJet_12345,"Acc+Trg+Imp+Qual+Iso","lp");
	LegEffRecoInclJ->AddEntry(&Eff_RecoInclJet_123456,"Acc+Trg+Imp+Qual+Iso+MuID","lp");
	...
	LegEffRecoExclJ->AddEntry(&Eff_RecoExclJet_1234,"Acc+Trg+Imp+Qual","lp");
	LegEffRecoExclJ->AddEntry(&Eff_RecoExclJet_12345,"Acc+Trg+Imp+Qual+Iso","lp");
	LegEffRecoExclJ->AddEntry(&Eff_RecoExclJet_123456,"Acc+Trg+Imp+Qual+Iso+MuID","lp");
...
/////////////////////Relative Efficiency from MC
	...
	LegREffRecoExclWGA->AddEntry(&RelEff_RecoExclJetWGA_1234,"Qual","lp");
	LegREffRecoExclWGA->AddEntry(&RelEff_RecoExclJetWGA_12345,"Iso","lp");
	LegREffRecoExclWGA->AddEntry(&RelEff_RecoExclJetWGA_123456,"MuID","lp");
	...
	LegREffRecoInclWGA->AddEntry(&RelEff_RecoInclJetWGA_1234,"Qual","lp");
	LegREffRecoInclWGA->AddEntry(&RelEff_RecoInclJetWGA_12345,"Iso","lp");
	LegREffRecoInclWGA->AddEntry(&RelEff_RecoInclJetWGA_123456,"MuID","lp");
	...
	LegREffRecoExclJ->AddEntry(&RelEff_RecoExclJet_1234,"Qual","lp");
	LegREffRecoExclJ->AddEntry(&RelEff_RecoExclJet_12345,"Iso","lp");
	LegREffRecoExclJ->AddEntry(&RelEff_RecoExclJet_123456,"MuID","lp");	
	...
	LegREffRecoInclJ->AddEntry(&RelEff_RecoInclJet_1234,"Qual","lp");
	LegREffRecoInclJ->AddEntry(&RelEff_RecoInclJet_12345,"Iso","lp");
	LegREffRecoInclJ->AddEntry(&RelEff_RecoInclJet_123456,"MuID","lp");	
	...
	effincl<<"Jet# >= "<<i<<" Relative Eff. = "<<y<<" Error High = "<<RelEff_RecoInclJet_123.GetErrorYhigh(i)<<" Error Low = "<<RelEff_RecoInclJet_123.GetErrorYlow(i)<<endl;}
        ...
        effincl<<endl<<"Selection: Qual"<<endl;
        for(int i=0; i<7; i++){
	double x = 0., y = 0.;
	...
	effincl<<endl<<"Selection: MuID"<<endl;
	for(int i=0; i<7; i++){
	double x = 0., y = 0.;
	...
	effexcl<<"Jet# = "<<i<<" Relative Eff. = "<<y<<" Error High = "<<RelEff_RecoExclJet_123.GetErrorYhigh(i)<<" Error Low = "<<RelEff_RecoExclJet_123.GetErrorYlow(i)<<endl;}
        ...
        effexcl<<endl<<"Selection: Qual"<<endl;
        for(int i=0; i<7; i++){
	double x = 0., y = 0.;
	...
	effexcl<<endl<<"Selection: MuID"<<endl;
	for(int i=0; i<7; i++){
	double x = 0., y = 0.;
...
///////////////////// Distributions
	...
	string recMassZ_name = "RecoMuon/recZ_Plots/recMassZ";
	...
	string recPtZ_name = "RecoMuon/recZ_Plots/recPtZ";
	...
	string recEtaZ_name = "RecoMuon/recZ_Plots/recEtaZ";
	...
	string recExclJetN_name = "RecoMuon/recJet_Plots/IsoJetCounter";
...
////////////////////////// Gen - Rec Plots
	...
	TH1D* GENMassZ_Acc = (TH1D*) signal_file->Get("GenMuon/genZ_Plots/genMassZ_Acc");
	TH1D* GENPtZ_Acc = (TH1D*) signal_file->Get("GenMuon/genZ_Plots/genPtZ_Acc");
	TH1D* GENEtaZ_Acc = (TH1D*) signal_file->Get("GenMuon/genZ_Plots/genEtaZ_Acc");
	TH1D* GENExclJetN_Acc = (TH1D*) signal_file->Get("GenMuon/genJet_Plots/GenIsoJetCounter_Acc");
...
/////////////////////// Jet Isolation
	...
	TH1D* GENExclJetPt = (TH1D*) signal_file->Get("GenMuon/genJet_Plots/GenJetPt_Acc");
	TH1D* GENIsoJetPt = (TH1D*) signal_file->Get("GenMuon/genJet_Plots/GenIsoJetPt_Acc");
	
	string RECJetPt_name = "RecoMuon/recJet_Plots/RecoJetPt";
	...
	string RECIsoJetPt_name = "RecoMuon/recJet_Plots/RecoIsoJetPt";
