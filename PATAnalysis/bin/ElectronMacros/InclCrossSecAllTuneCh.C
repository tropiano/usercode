...
void XSec(int JetPtMin, string tune, string jecUnc){
	...
	double iniLumiNotNormPythia = 1277.08; // Z Pythia sample
	
	double scaleGENRECO = Luminosity/iniLumiNotNorm;
	double scaleGENRECOPythia = Luminosity/iniLumiNotNormPythia;
	...
	////////////////////////// Directories /////////////////////////////////////////////
        ...
        TDirectory *multdir, *yieldsdir, *tpeffdir, *mceffdir, *corrfactdir, *xsecdir, *corrmultdir, *effcorrdir;
        ...
        corrmultdir = multdir->mkdir("Multiplicity");
        ...
        TFile *UNFpythia = TFile::Open((path+"/Ntuple/"+JetPtCut+"/Z2/NtuplePlots_Pythia_"+JetPtCut+".root").c_str());
        ...
        TH1D* GENMultPythia_incl = (TH1D*) UNFpythia->Get("Unfolding/CorrFactors/GENJet_InclMult");
        GENMultPythia_incl->Sumw2();
        GENMultPythia_incl->Scale(scaleGENRECOPythia);
        GENMultPythia_incl->Write("GENMultPythia_incl");
        effcorrdir->cd();
        ...
        TGraphAsymmErrors* Efficiency_graph = new TGraphAsymmErrors();
        effcorrdir->cd();
        ...
        double EffX_incl, EffY_incl, EffY_incl_errh, EffY_incl_errl;
        double RelUncEff_High, RelUncEff_Low;
        ...
        EffX_incl=0.;
        EffY_incl=0.;
        EffY_incl_errh=0.; EffY_incl_errl=0.;
        
        RelUncEff_High=0.; RelUncEff_Low=0.;
        ...
        EffX_incl=xmceff_incl;
        EffY_incl=ymceff_incl*(ytpdata_incl/ytpmc_incl);
        multrep<<"(MCGlobalEff)*Rho = "<<EffY_incl<<endl;
        ...
        EffCorrMult_incl = (ys_incl-BinWZ_ZZ_incl)/EffY_incl;
        BinCorrMult_incl = EffCorrMult_incl*BinUCF_incl;
        
        //Statistical  relative uncertainty on Nsig 
        RelStatUnc=ys_incl_err/(ys_incl-BinWZ_ZZ_incl);
        
        BinCorrMult_incl_errh = BinCorrMult_incl*RelStatUnc;        
        BinCorrMult_incl_errl = BinCorrMult_incl*RelStatUnc;
        
        multrep<<"Corrected Incl. Multiplcity = "<<BinCorrMult_incl<<" (+) "<<BinCorrMult_incl_errh<<" (-) "<<BinCorrMult_incl_errl<<endl<<endl;
        
        multrep<<"STATISTICAL RELATIVE unc. (Nsig) = 	"<<RelStatUnc<<endl<<endl;
        
        multrep<<"SYSTEMATIC RELATIVE uncertainties:"<<endl<<endl;
        multrep<<"REL. UNC. WZ-ZZ background =			"<<BinWZ_ZZ_incl_err/(ys_incl-BinWZ_ZZ_incl)<<endl<<endl;
        multrep<<"REL. UNC. TP Eff. Data =		"<<" (High) "<<ytpdata_incl_errh/ytpdata_incl<<" (Low) "<<ytpdata_incl_errl/ytpdata_incl<<endl;
        multrep<<"REL. UNC. TP Eff. MC =			"<<" (High) "<<ytpmc_incl_errh/ytpmc_incl<<" (Low) "<<ytpmc_incl_errl/ytpmc_incl<<endl;
        multrep<<"REL. UNC. MC Global Eff. =		"<<" (High) "<<ymceff_incl_errh/ymceff_incl<<" (Low) "<<ymceff_incl_errl/ymceff_incl<<endl<<endl;
        multrep<<"REL. UNC. Luminosity =				"<<LuminosityErr/Luminosity<<endl<<endl;
        
        multrep<<">>>>> Efficiency Value and Uncertainty <<<<<<<"<<endl<<endl;
        
        //NB: inverto errore high e low di EpsilonTP perchè è a denominatore
        RelUncEff_High = sqrt(pow(ymceff_incl_errh/ymceff_incl,2)+pow(ytpmc_incl_errl/ytpmc_incl,2)+pow(ytpdata_incl_errh/ytpdata_incl,2));
        RelUncEff_Low = sqrt(pow(ymceff_incl_errl/ymceff_incl,2)+pow(ytpmc_incl_errh/ytpmc_incl,2)+pow(ytpdata_incl_errl/ytpdata_incl,2));
        
        EffY_incl_errh = RelUncEff_High*EffY_incl;
        EffY_incl_errl = RelUncEff_Low*EffY_incl;
        
        multrep<<"(MCGlobalEff)*Rho = "<<EffY_incl<<" + "<<EffY_incl_errh<<" - "<<EffY_incl_errl<<endl<<endl;
        
        multrep<<"REL. UNC. (MCGlobalEff)*Rho =		"<<" (High) "<<RelUncEff_High<<" (Low) "<<RelUncEff_Low<<endl<<endl;
        
        Efficiency_graph->SetPoint(i, i, EffY_incl);
        Efficiency_graph->SetPointEYhigh(i, EffY_incl_errh);
        Efficiency_graph->SetPointEYlow(i, EffY_incl_errl);
        Efficiency_graph->SetPointEXhigh(i, 0.5);
        Efficiency_graph->SetPointEXlow(i, 0.5);
        ...
        Efficiency_graph->Write("Efficiency_graph");
        effcorrdir->cd();
        ...
        TGraphAsymmErrors* INCLGENMultPythia_graph = new TGraphAsymmErrors();
        
        for(int i=0;i<bin_incl;i++){
        ygmult=0.;
        yerr=0.;
        ygmult = GENMultPythia_incl->GetBinContent(i+1);
        yerr = GENMultPythia_incl->GetBinError(i+1);
        INCLGENMultPythia_graph->SetPoint(i, i, ygmult);
        INCLGENMultPythia_graph->SetPointEYhigh(i, yerr);
        INCLGENMultPythia_graph->SetPointEYlow(i, yerr);
        INCLGENMultPythia_graph->SetPointEXhigh(i, 0.5);
        INCLGENMultPythia_graph->SetPointEXlow(i, 0.5);}
        ...
        INCLUnfMultRatio_graph->Write("INCLUnfMultRatio_graph");
        ...
        INCLGENMult_graph->GetXaxis()->SetTitle("inclusive multiplicity");
        INCLGENMult_graph->GetXaxis()->SetRangeUser(-0.7,bin_incl-0.3);
        INCLGENMult_graph->GetXaxis()->SetNdivisions(6);
        INCLGENMult_graph->GetYaxis()->SetTitle("N(Z + #geq n jets)");
        INCLGENMult_graph->GetYaxis()->SetRangeUser(10,50000);
        INCLGENMult_graph->Draw("AP");
        INCLGENMult_graph->SetTitle("GEN Mult. (red) - Pythia Mult. (yellow) - DATA Unf.Mult. (black)");
        INCLGENMultPythia_graph->SetLineColor(807);
        INCLGENMultPythia_graph->SetLineWidth(2);
        INCLGENMultPythia_graph->SetMarkerStyle(25);
        INCLGENMultPythia_graph->SetMarkerColor(807);
        INCLGENMultPythia_graph->Draw("PSAME");          	
	...
	INCLCorrMult_graph->SetMarkerStyle(22);
        ...
        INCLmultRatio->SetCanvasSize(600,300);
        INCLmultRatio->SetGridx();
        INCLmultRatio->SetGridy();
        INCLUnfMultRatio_graph->SetLineColor(1);
        INCLUnfMultRatio_graph->SetLineWidth(2);
        INCLUnfMultRatio_graph->SetMarkerStyle(22);
        INCLUnfMultRatio_graph->GetXaxis()->SetTitle("inclusive multiplicity");
        INCLUnfMultRatio_graph->GetXaxis()->SetRangeUser(-0.7,bin_incl-0.3);
        INCLUnfMultRatio_graph->GetXaxis()->SetNdivisions(6);
        INCLUnfMultRatio_graph->GetYaxis()->SetRangeUser(0.4,1.6);
        INCLUnfMultRatio_graph->GetYaxis()->SetTitle("#frac{N_{Data}(Z + #geq n jets)}{N_{MC}(Z + #geq n jets)}");
        INCLUnfMultRatio_graph->GetYaxis()->CenterTitle();
        INCLUnfMultRatio_graph->GetYaxis()->SetTitleSize(0.05);
        INCLUnfMultRatio_graph->GetYaxis()->SetTitleOffset(1.2);
        ...
        INCLRECOMult_graph->GetPoint(i, xreco, yreco);
        yreco_err = INCLRECOMult_graph->GetErrorY(i);
       
        INCLEffMultRatio_graph->SetPoint(i, i, yunf/yreco);
        ratio_errh = (yunf/ygen)*sqrt(pow((yunf_errh/yunf),2)+pow((yreco_err/ygen),2));
        ratio_errl = (yunf/ygen)*sqrt(pow((yunf_errl/yunf),2)+pow((yreco_err/ygen),2));
...
///////////////////////////////// Cross Sections //////////////////////////////////////////
	...
	//L'errore sulla luminosità viene aggiunto con FinalXSec
	xsections_errh = xsections*sqrt(ymult_errh/ymult);
	xsections_errl = xsections*sqrt(ymult_errl/ymult);
	...
	//NB: inverto errore high e low di INCL_ymult perchè è a denominatore
	xsec_ratio_errh=(ymult/INCL_ymult)*sqrt(pow((ymult_errh/ymult),2)+pow((INCL_ymult_errl/INCL_ymult),2));
	xsec_ratio_errl=(ymult/INCL_ymult)*sqrt(pow((ymult_errl/ymult),2)+pow((INCL_ymult_errh/INCL_ymult),2));
