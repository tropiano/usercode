...
nameSource = ['file:test_Summer11.root']   # Input file name
#nameSource = ['file:test_dati2011.root'] 
numEventsToRun = -1                       # (e.g. -1 to run on all events)
nameOutput = 'output_Summer11.root'        # Output file name
...
process.recosequence = cms.Sequence(process.L1FastCorrection*
...
if isMC == True:
	...
	process.pattuples = cms.Sequence(process.goodTracks + process.recosequence)
	if DataSkimm == True:	
		process.p = cms.Path(process.pattuples * process.skimPatMuons)
	else:
		process.p = cms.Path(process.pattuples)
