...
namespace
{
  ...
  {
    return !makeCaloJet (fTag) && !makePFJet (fTag) && !makeGenJet (fTag)
      && !makeBasicJet (fTag);
  }

  template < class T > void dumpJets (const T & fJets)
  {
  ...
  void copyConstituents (const JetReco::InputCollection & fConstituents,
			 const edm::View < Candidate > &fInput,
			 reco::Jet * fJet)
  {
    // put constituents
    for (unsigned iConstituent = 0; iConstituent < fConstituents.size ();
	 ++iConstituent)
      {
	fJet->addDaughter (fInput.
			   ptrAt (fConstituents[iConstituent].index ()));
      }
  }
...
namespace cms
{
  ...
  ExclInclFastJetProducer::ExclInclFastJetProducer (const edm::
						    ParameterSet &
						    conf):mSrc (conf.
								getParameter <
								edm::
  ...
  {
    std::string alias =
      conf.getUntrackedParameter < string > ("alias",
	...
	produces < CaloJetCollection > ().setBranchAlias (alias);
        ...
        produces < PFJetCollection > ().setBranchAlias (alias);
    else if (makeGenJet (mJetType))
      produces < GenJetCollection > ().setBranchAlias (alias);
...
//     else if (makeGenericJet (mJetType)) produces<GenericJetCollection>().setBranchAlias (alias);
  ...
  ExclInclFastJetProducer::~ExclInclFastJetProducer ()
  {
  }

  // Functions that gets called by framework every event
  void ExclInclFastJetProducer::produce (edm::Event & e,
					 const edm::EventSetup & fSetup)
  {
	...
	std::
	  cout <<
	  "ExclInclFastJetProducer::produce-> INPUT COLLECTION selected from"
	  << mSrc << " with ET > " << mEtInputCut << " and/or E > " <<
	  mEInputCut << std::endl;
	for (unsigned index = 0; index < input.size (); ++index)
	  {
	    std::cout << "  Input " << index << ", px/py/pz/pt/e: "
	      << input[index]->px () << '/' << input[index]->
	      py () << '/' << input[index]->pz () << '/' << input[index]->
	      pt () << '/' << input[index]->energy () << std::endl;
	  }
      }
	...
	const CaloSubdetectorGeometry *towerGeometry =
	  geometry->getSubdetectorGeometry (DetId::Calo,
	...
	for (unsigned iJet = 0; iJet < output.size (); ++iJet)
	  {
	    ...
	    if (protojet->p4 ().pt () < mJetPtMin)
	      continue;
	    const JetReco::InputCollection & constituents =
...
//     }
  ...
  bool ExclInclFastJetProducer::runAlgorithm (const InputCollection& fInput, 
  ...
  DEFINE_FWK_MODULE (ExclInclFastJetProducer);
}
